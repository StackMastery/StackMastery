      ...
      >
        ...
        >
          <div className="flex w-full items-center flex-col max-w-primary border-x border-slate-800">
            ...
            <div className="w-full p-5 gap-5 min-h-[500px] grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
              <AnimatePresence>
                {!projects &&
                  ...
                  projects.slice(0, 3).map((project, index) => (
                    ...
                    >
                      <AspectRatio
                        ...
                        ratio={3 / 2}
                      >
                        <Link to={`/projects${project?.permallink}`}>
                          <img
                            src={project.image}
                            alt={project.title}
                      ...
                      <div>
                        ...
                        <div className="flex gap-x-2 gap-y-1 flex-wrap py-2 text-sm">
                          {project.technology?.map((tech, index) => (
                            <button
                              key={`tech-${index + 1}`}
                              className="bg-white/5 border border-white/10 px-3"
                            >
                              {tech}
                            </button>
                          ))}
                        </div>
                        <div className="w-full flex gap-3 pt-5 justify-between max-[380px]:flex-wrap">
                          <div className="flex gap-3 max-[380px]:w-full">
                            <Link
                              target="_blank"
                              to={project.github}
                              className="bg-white/5 border border-white/10 p-2 px-3 rounded-md max-[380px]:w-full flex justify-center"
                            >
                              <FaGithub size={20} />
                            </Link>
                            <Link
                          ...
                          <Link
                            to={`/projects${project?.permallink}`}
                            className="bg-white/5 border border-white/10 p-2 px-5 rounded-md max-[380px]:w-full text-center"
                          >
                            View Details
                          </Link>
                        </div>
